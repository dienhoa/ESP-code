...
//  str.reserve(dataLength+15);
  ...
  while (str.length()<dataLength) {
    if(Serial.available()){
      ...
      }else if(((c>=0x20)&&(c<=0x7E))||(c=='\n')||(c=='\r')){ // do not take the STX and ETX
        str += c; 
      }
...
//  str.replace("\\r","\\\r"); // need to check again
  ...
  str.replace("\n", " ");
  str.replace("\r", " ");
  str.replace("\\", "S-bs"); // double the special characters to make client.print accept
 
  str.replace("\"","S-g");
  str.replace("\'","\'\'");
//  for (int i;i<=str.length();i++){
//    if ((str.charAt(i)<0x20)||(str.charAt(i)>0x7E)){data.setCharAt(i,' ');}
//  }
  str = "{\"value\":\"" + str +"\"}";
//  data += str;
//  data += "\"}";;
